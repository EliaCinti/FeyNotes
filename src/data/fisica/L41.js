const LESSON = {
    ...
    abstract: "Passaggio dalla meccanica alla termodinamica: sistemi termodinamici, temperatura, stati di equilibrio, trasformazioni quasi-statiche e reversibili, Principio Zero, esperimento di Joule, energia interna e formulazione del Primo Principio con applicazioni alle trasformazioni speciali.",
    ...
    sections: [
        {
            ...
            icon: "🔄",
            content: `<p>In dinamica siamo arrivati a definire una proprietà dei sistemi: la loro <strong>capacità di compiere lavoro</strong>. Questa capacità è quantificata come energia immagazzinata, che può essere configurazionale (potenziale) o in forma di moto (cinetica). La somma di queste energie è l'energia meccanica.</p>
...
<p>Qui $L_{\\text{tot}}$ indica il lavoro compiuto da <em>tutte</em> le forze agenti sul sistema, sia conservative sia non conservative. Questa relazione è nota come <strong>Teorema dell'energia cinetica</strong> (o Teorema lavoro-energia): la variazione di energia cinetica di un corpo è uguale al lavoro netto compiuto su di esso dalla risultante di tutte le forze.</p>
<p>Questo è il nostro punto di partenza. Le forze, agendo su un sistema in movimento, possono conferirgli o sottrargli la capacità di mettere in moto altri sistemi, cioè di compiere lavoro. Questa capacità viene accumulata dal sistema, aumentando il suo contenuto energetico.</p>`,
            subsections: [
                {
                    subtitle: "Il problema dell'energia 'perduta'",
                    content: `<p>In meccanica, se agiscono solo forze conservative, l'energia meccanica totale $E = E_k + E_p$ si conserva. Quando però sono presenti forze non conservative (come l'attrito), l'energia meccanica non si conserva: essa diminuisce e si "perde".</p>
<p>La termodinamica nasce proprio per dare conto di questa energia apparentemente scomparsa, riconoscendo che essa si è trasformata in <strong>energia interna (termica)</strong> del sistema.</p>
<p>La termodinamica estende dunque i concetti della meccanica per includere non solo le trasformazioni meccaniche, ma anche quelle termiche, fornendo un quadro completo della conservazione dell'energia.</p>`
                }
            ],
            formulas: [
...
<p>Nella maggior parte dei casi tratteremo sistemi chiusi.</p>
...
<p>Un punto fondamentale della termodinamica è che essa descrive i sistemi attraverso le loro <strong>proprietà macroscopiche</strong>. Queste proprietà, che possiamo misurare su larga scala, sono in realtà la manifestazione di fenomeni che avvengono a livello microscopico.</p>`,
            subsections: [
                {
                    subtitle: "Esempio: la pressione di un gas",
                    content: `<p>Consideriamo la <strong>pressione</strong> esercitata da un gas sulle pareti di un recipiente. A livello macroscopico, osserviamo una forza distribuita sulla parete, capace di spostarla. A livello microscopico, questa forza è l'effetto cooperativo di miliardi di miliardi di urti delle singole molecole del gas contro la parete.</p>
<p>Ogni singola molecola, rimbalzando sulla parete, le trasferisce una piccola quantità di moto; la somma di tutti questi contributi produce la forza macroscopica che misuriamo come pressione. La termodinamica non studia il singolo urto della singola molecola, ma l'effetto macroscopico complessivo.</p>`
                },
                {
            ...
            subsections: [
                {
                    ...
                    content: `<ul>
<li>In un <strong>gas</strong>, una temperatura più alta corrisponde a una maggiore velocità media delle molecole, e quindi a una maggiore energia cinetica media. Per un gas ideale monoatomico, la relazione precisa è:</li>
</ul>
<p>$$\\langle E_k \\rangle = \\frac{3}{2}\\,k_B\\,T$$</p>
<p>dove $k_B \\approx 1{,}381 \\times 10^{-23}\\,\\text{J/K}$ è la costante di Boltzmann.</p>
<ul>
...
<p>In entrambi i casi, la temperatura è l'evidenza macroscopica dello stato cinetico microscopico del sistema. La temperatura è una grandezza <strong>intensiva</strong>: non dipende dalla quantità di materia.</p>`
                },
                {
...
<p>Le grandezze macroscopiche che individuano lo stato di un sistema sono dette <strong>variabili di stato</strong> (o funzioni di stato). Il loro valore dipende esclusivamente dallo stato in cui si trova il sistema e <strong>non dalla storia passata</strong>, cioè dal particolare percorso attraverso cui il sistema ha raggiunto quello stato.</p>`,
            subsections: [
                {
...
<p>Se un sistema non è in equilibrio (ad esempio, ha temperature diverse in punti diversi), esso evolverà spontaneamente fino a raggiungere una nuova condizione di equilibrio.</p>`
        ...
        },
        {
            ...
            subsections: [
                ...
                {
                    subtitle: "Reversibilità: un punto delicato",
                    content: `<p>La reversibilità <strong>non</strong> significa semplicemente che si può tornare allo stato iniziale. Anche dopo una trasformazione irreversibile si può riportare il sistema allo stato iniziale, ma per farlo occorre coinvolgere l'ambiente, che risulterà modificato.</p>
<p>La reversibilità richiede che si possa tornare allo stato iniziale facendo ripercorrere al sistema <em>e all'ambiente</em> la stessa sequenza di stati intermedi in ordine inverso, senza alcuna modifica residua.</p>
<p><strong>Da ricordare:</strong> tutte le trasformazioni reali sono irreversibili. Le trasformazioni reversibili sono un'<strong>idealizzazione</strong> utile per il calcolo e per stabilire limiti teorici alle prestazioni dei processi.</p>`
                }
            ]
        },
        {
...
<ul>
<li><strong>Trasformazione isoterma</strong>: avviene a <strong>temperatura costante</strong> ($T = \\text{cost}$). Per realizzarla, il sistema deve poter scambiare calore con un serbatoio termico (termostato) a temperatura fissata.</li>
<li><strong>Trasformazione isobara</strong>: avviene a <strong>pressione costante</strong> ($P = \\text{cost}$). Ad esempio, un gas che si espande in un cilindro con pistone mobile sottoposto alla pressione atmosferica costante.</li>
<li><strong>Trasformazione isocora</strong> (o isovolumetrica): avviene a <strong>volume costante</strong> ($V = \\text{cost}$). Poiché il volume non cambia, il sistema non compie lavoro meccanico ($L = 0$).</li>
<li><strong>Trasformazione adiabatica</strong>: il sistema <strong>non scambia calore</strong> con l'ambiente ($Q = 0$). Ciò si realizza con pareti termicamente isolanti oppure con processi così rapidi da non consentire scambi termici apprezzabili.</li>
</ul>`,
            table_compare: {
                headers: ["Trasformazione", "Grandezza costante", "Conseguenza"],
                rows: [
                    ["Isoterma", "$T = \\text{cost}$", "$\\Delta U = 0$ (gas ideale)"],
                    ["Isobara", "$P = \\text{cost}$", "$L = P\\,\\Delta V$"],
                    ["Isocora", "$V = \\text{cost}$", "$L = 0$"],
...
<p>Questo principio, apparentemente ovvio, è in realtà non banale e non deducibile dagli altri principi della termodinamica. Esso stabilisce la <strong>transitività dell'equilibrio termico</strong> e garantisce che la temperatura sia una proprietà ben definita e misurabile: due sistemi qualsiasi che siano ciascuno in equilibrio termico con un terzo sistema, sono necessariamente in equilibrio termico tra loro e hanno la stessa temperatura.</p>`,
            subsections: [
                {
...
<li>Si osserva che la temperatura dell'acqua <strong>aumenta</strong>.</li>
</ol>
...
<p>L'esperimento dimostrò definitivamente che calore e lavoro sono due forme equivalenti di trasferimento di energia, misurabili con la stessa unità (il joule nel Sistema Internazionale).</p>`
        ...
        },
        {
            ...
            type: "section",
            title: "Calore e Lavoro: Energie in Transito",
            icon: "🔥",
            content: `<p>L'esperimento di Joule stabilisce un'equivalenza fondamentale: il lavoro e lo scambio termico sono due modi diversi per produrre lo stesso effetto, cioè una variazione dello stato termico.</p>
<p>Il <strong>calore</strong> ($Q$) è l'energia trasferita tra due sistemi (o tra un sistema e il suo ambiente) a causa di una <strong>differenza di temperatura</strong> tra essi.</p>`,
            subsections: [
                {
                    subtitle: "Punto chiave: calore e lavoro non sono contenuti in un corpo",
                    content: `<p>Sia il lavoro ($L$) sia il calore ($Q$) <strong>non</strong> sono energie <em>contenute</em> in un sistema, ma energie <em>in transito</em>, cioè modalità di trasferimento di energia.</p>
<p>È scorretto dire che "un corpo contiene calore". Un corpo possiede energia interna, e può scambiare questa energia sotto forma di calore o lavoro. Il calore e il lavoro esistono solo <em>durante</em> un processo di trasferimento.</p>`
                },
                {
                    subtitle: "Convenzione dei segni",
                    content: `<p><strong>Calore ($Q$):</strong></p>
<ul>
...
<li>$L \\lt 0$: l'ambiente compie lavoro <strong>sul</strong> sistema (es. un gas che viene compresso da una forza esterna).</li>
  ...
  <p><em>Riquadro centrale "Sistema" con frecce: a sinistra "Ambiente termico" con frecce Q>0 (entrante, rosso) e Q<0 (uscente, rosso); a destra "Ambiente meccanico" con frecce L>0 (uscente, blu) e L<0 (entrante, blu).</em></p>
  <p style="margin-top: 8px; font-size: 0.75rem; color: var(--accent);">[ immagine da inserire ]</p>
</div>
<p>Questa convenzione è detta "convenzione del sistema", poiché assume il punto di vista del sistema: è positivo ciò che aumenta l'energia del sistema (calore entrante) o ciò che il sistema fa sull'esterno (lavoro uscente). Alcuni testi adottano la convenzione opposta per il lavoro (positivo quando è fatto <em>sul</em> sistema), nel qual caso il Primo Principio si scrive $\\Delta U = Q + L$. È fondamentale verificare quale convenzione viene usata prima di applicare le formule.</p>`
        },
        {
...
<p>In entrambi i casi la variazione di energia interna $\\Delta U = U_B - U_A$ è la stessa, perché $U$ è una funzione di stato. Tuttavia, i valori di $Q$ e $L$ lungo i due cammini saranno in generale diversi: cambia la ripartizione tra calore e lavoro, ma la loro differenza $Q - L$ rimane costante.</p>`
                }
            ]
        },
        {
...
<p>Il Primo Principio afferma che queste sono le <em>uniche</em> modalità di scambio energetico per un sistema chiuso, e che il loro bilancio netto determina la variazione di energia interna.</p>`
        ...
        {
            id: "s41-applicazioni-primo",
            type: "section",
            title: "Applicazione del Primo Principio alle Trasformazioni Speciali",
            icon: "🧮",
            content: `<p>Possiamo ora applicare il Primo Principio $\\Delta U = Q - L$ alle trasformazioni speciali introdotte in precedenza.</p>`,
            subsections: [
                {
                    subtitle: "Trasformazione isocora (V = cost, L = 0)",
                    content: `<p>$$\\Delta U = Q$$</p>
<p>Tutto il calore scambiato va a variare l'energia interna del sistema. Questa relazione permette di misurare variazioni di energia interna in modo diretto.</p>`
                },
                {
...
<p>L'energia interna varia solo a spese del lavoro. Se il sistema compie lavoro ($L \\gt 0$), la sua energia interna diminuisce e la temperatura scende. Viceversa, se l'ambiente compie lavoro sul sistema ($L \\lt 0$), l'energia interna aumenta e la temperatura sale.</p>`
                },
                {
                    ...
                    content: `<p>Per un gas ideale l'energia interna dipende solo dalla temperatura, quindi $\\Delta U = 0$ e:</p>
<p>$$Q = L$$</p>
...
<p>$$\\Delta U = Q - P\\,\\Delta V$$</p>`
        ...
        },
        {
            ...
            icon: "📐",
            content: `<p>Consideriamo $n = 2\\,\\text{mol}$ di gas ideale monoatomico (per il quale $\\Delta U = \\frac{3}{2}\\,n\\,R\\,\\Delta T$) che si espandono a pressione costante $P = 1{,}0 \\times 10^5\\,\\text{Pa}$. La temperatura passa da $T_i = 300\\,\\text{K}$ a $T_f = 600\\,\\text{K}$.</p>`,
            subsections: [
                {
...
<p>$$Q = \\Delta U + L = 7483 + 4989 \\approx 12\\,472\\,\\text{J}$$</p>`
                },
                {
                    ...
                    content: `<p>Il calore assorbito ($Q \\approx 12\\,472\\,\\text{J}$) è maggiore della variazione di energia interna ($\\Delta U \\approx 7483\\,\\text{J}$). Questo perché il calore fornito al sistema si ripartisce in due contributi:</p>
<ul>
...
<p>Dal Primo Principio, con $Q = 0$:</p>
<p>$$\\Delta U = Q - L = 0 - (-500) = +500\\,\\text{J}$$</p>
<p>L'energia interna del gas aumenta di $500\\,\\text{J}$. Possiamo calcolare l'aumento di temperatura:</p>
<p>$$\\Delta T = \\frac{\\Delta U}{\\frac{3}{2}\\,n\\,R} = \\frac{500}{\\frac{3}{2} \\times 1 \\times 8{,}314} \\approx 40{,}1\\,\\text{K}$$</p>
<p>Nella compressione adiabatica il gas si riscalda, poiché tutto il lavoro fatto su di esso si converte in energia interna.</p>`
        },
        {
            id: "s41-nota-cicliche",
            type: "note_box",
            title: "Trasformazioni cicliche e prosieguo",
            icon: "🔜",
            content: `<p>La trattazione rigorosa del Primo Principio, a partire dalla sua formulazione storica e dalle sue conseguenze per le trasformazioni cicliche (in cui $\\Delta U = 0$ e quindi $Q_{\\text{ciclo}} = L_{\\text{ciclo}}$), verrà approfondita nelle lezioni successive.</p>`
        }
    ],
    ...
    oral_cards: [
        {
            type: "definizione",
            front: "Che cos'è un sistema termodinamico? Quali tipi si distinguono in base alla frontiera?",
            back: "Un <strong>sistema termodinamico</strong> è una porzione di spazio e materia delimitata da una frontiera che la separa dall'ambiente. Si distinguono: <strong>sistema aperto</strong> (scambia materia ed energia), <strong>sistema chiuso</strong> (scambia energia ma non materia), <strong>sistema isolato</strong> (non scambia né materia né energia)."
        },
        {
            type: "definizione",
            front: "Che cos'è la temperatura dal punto di vista microscopico?",
            back: "La temperatura è una misura dell'agitazione termica dei costituenti microscopici del sistema. In un gas ideale monoatomico: $\\langle E_k \\rangle = \\frac{3}{2}\\,k_B\\,T$. In un solido, corrisponde all'ampiezza delle vibrazioni degli atomi attorno alle posizioni di equilibrio. È una grandezza <strong>intensiva</strong>."
        },
        {
            type: "definizione",
            front: "Che cos'è lo stato di equilibrio termodinamico? Quali condizioni deve soddisfare?",
            back: "Un sistema è in equilibrio termodinamico quando le sue proprietà macroscopiche sono uniformi in ogni punto e non variano nel tempo. Deve sussistere simultaneamente: <strong>equilibrio termico</strong> (temperatura uniforme), <strong>equilibrio meccanico</strong> (pressione uniforme), <strong>equilibrio chimico</strong> (composizione costante)."
        },
        {
            type: "domanda",
            front: "Qual è la differenza tra trasformazione reversibile e irreversibile?",
            back: "Una trasformazione <strong>reversibile</strong> è quasi-statica e priva di fenomeni dissipativi: si può invertire ripercorrendo gli stessi stati intermedi, riportando sistema <em>e ambiente</em> alle condizioni iniziali. Una trasformazione <strong>irreversibile</strong> avviene in tempo finito, passa per stati di non-equilibrio, e non può essere invertita senza modificare l'ambiente. Tutte le trasformazioni reali sono irreversibili."
        },
        {
            type: "domanda",
            front: "Enuncia il Principio Zero della Termodinamica e spiega perché è importante.",
            back: "Se $A$ è in equilibrio termico con $C$ e $B$ è in equilibrio termico con $C$, allora $A$ e $B$ sono in equilibrio termico tra loro ($T_A = T_B$). Stabilisce la <strong>transitività dell'equilibrio termico</strong> e garantisce che la temperatura sia misurabile: è ciò che giustifica il funzionamento del termometro."
        },
        {
            type: "tranello",
            front: "Perché è scorretto dire che 'un corpo contiene calore'?",
            back: "Calore e lavoro <strong>non</strong> sono energie contenute in un sistema, ma energie <em>in transito</em>: esistono solo durante un processo di trasferimento. Un corpo possiede <strong>energia interna</strong> $U$, e può scambiare energia sotto forma di calore $Q$ o lavoro $L$."
        },
        {
            type: "formula",
            front: "Scrivi il Primo Principio della Termodinamica e la sua forma differenziale. Quale convenzione dei segni si usa?",
            back: "$\\Delta U = Q - L$ (forma finita), $dU = \\delta Q - \\delta L$ (forma differenziale). Convenzione: $Q \\gt 0$ = calore assorbito dal sistema; $L \\gt 0$ = lavoro compiuto dal sistema sull'ambiente. $dU$ è differenziale esatto, $\\delta Q$ e $\\delta L$ no."
        },
        {
            type: "domanda",
            front: "Perché l'energia interna è una funzione di stato mentre calore e lavoro no?",
            back: "$U$ dipende solo dallo stato termodinamico attuale ($P, V, T, ...$) e non dal percorso: $\\Delta U = U_f - U_i$ è univocamente determinato dagli stati iniziale e finale. $Q$ e $L$ invece dipendono dal cammino della trasformazione: due percorsi diversi tra gli stessi stati danno valori diversi di $Q$ e $L$, ma la differenza $Q - L = \\Delta U$ resta la stessa."
        },
        {
            type: "formula",
            front: "Come si semplifica il Primo Principio per le quattro trasformazioni speciali (isocora, adiabatica, isoterma di gas ideale, isobara)?",
            back: "<strong>Isocora</strong> ($V = \\text{cost}$, $L = 0$): $\\Delta U = Q$. <strong>Adiabatica</strong> ($Q = 0$): $\\Delta U = -L$. <strong>Isoterma gas ideale</strong> ($T = \\text{cost}$, $\\Delta U = 0$): $Q = L$. <strong>Isobara</strong> ($P = \\text{cost}$): $\\Delta U = Q - P\\,\\Delta V$."
        },
        {
            type: "domanda",
            front: "Cosa dimostrò l'esperimento di Joule e qual è il valore dell'equivalente meccanico del calore?",
            back: "Joule dimostrò che lavoro meccanico e contatto termico sono due modi equivalenti di trasferire energia a un sistema, producendo lo stesso effetto (aumento di temperatura). L'equivalente meccanico del calore è $1\\,\\text{cal} \\approx 4{,}186\\,\\text{J}$: calore e lavoro sono misurabili con la stessa unità."
        },
        {
            type: "domanda",
            front: "In una compressione adiabatica, il gas si scalda o si raffredda? Perché?",
            back: "Il gas si <strong>scalda</strong>. Con $Q = 0$ il Primo Principio dà $\\Delta U = -L$. In una compressione $L \\lt 0$ (lavoro fatto <em>sul</em> sistema), quindi $\\Delta U \\gt 0$: l'energia interna aumenta e, con essa, la temperatura."
        }
    ]
};
