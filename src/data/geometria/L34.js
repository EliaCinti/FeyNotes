...
<p>$$B_i = \\{\\vec{v}_{1i}, \\dots, \\vec{v}_{L_i, i}\\}$$</p>
...
<p>$$B = \\bigcup_{i=1}^{r} B_i = \\{\\vec{v}_{ji} \\mid i=1, \\dots, r, \\quad j=1, \\dots, L_i\\}$$</p>
...
<p>Se $r = 1$, abbiamo un solo autovalore $\\lambda_1$. L'insieme di vettori è $B = B_1 = \\{\\vec{v}_{11}, \\dots, \\vec{v}_{L_1, 1}\\}$.</p>
<p>La tesi coincide esattamente con l'ipotesi: $B = B_1$ è per ipotesi un insieme di autovettori linearmente indipendenti, quindi $B$ è linearmente indipendente. ✓</p>`
                },
                {
...
<p>Prendiamo una combinazione lineare uguale a zero di tutti i vettori dell'insieme $B$:</p>
<p>$$\\sum_{i=1}^{r} \\sum_{j=1}^{L_i} a_{ji} \\vec{v}_{ji} = \\vec{0} \\qquad (\\star)$$</p>
<p>Vogliamo dimostrare che <strong>tutti</strong> i coefficienti $a_{ji}$ sono nulli.</p>
<p><strong>Passo 1:</strong> Applichiamo l'endomorfismo $F$ a entrambi i membri di $(\\star)$. Per la linearità di $F$ e usando $F(\\vec{v}_{ji}) = \\lambda_i \\vec{v}_{ji}$:</p>
<p>$$\\sum_{i=1}^{r} \\sum_{j=1}^{L_i} a_{ji} \\lambda_i \\vec{v}_{ji} = \\vec{0} \\qquad (\\star\\star)$$</p>
<p><strong>Passo 2:</strong> Moltiplichiamo $(\\star)$ per $\\lambda_1$:</p>
<p>$$\\sum_{i=1}^{r} \\sum_{j=1}^{L_i} a_{ji} \\lambda_1 \\vec{v}_{ji} = \\vec{0} \\qquad (\\star\\star\\star)$$</p>
<p><strong>Passo 3:</strong> Sottraiamo $(\\star\\star\\star)$ da $(\\star\\star)$:</p>
<p>$$\\sum_{i=1}^{r} \\sum_{j=1}^{L_i} a_{ji} (\\lambda_i - \\lambda_1) \\vec{v}_{ji} = \\vec{0}$$</p>
<p>Per $i = 1$, il fattore $(\\lambda_1 - \\lambda_1) = 0$ annulla tutti i termini corrispondenti. Resta:</p>
<p>$$\\sum_{i=2}^{r} \\sum_{j=1}^{L_i} a_{ji} (\\lambda_i - \\lambda_1) \\vec{v}_{ji} = \\vec{0}$$</p>
...
<p>$$\\sum_{j=1}^{L_1} a_{j1} \\vec{v}_{j1} = \\vec{0}$$</p>
<p>Ma i vettori $\\{\\vec{v}_{11}, \\dots, \\vec{v}_{L_1, 1}\\}$ sono linearmente indipendenti per ipotesi. Quindi anche $a_{j1} = 0$ per ogni $j$.</p>
<p>Avendo dimostrato che <strong>tutti</strong> i coefficienti $a_{ji}$ sono nulli, l'insieme $B$ è linearmente indipendente. $\\square$</p>`
                }
            ],
            formulas: [
                { label: "Unione delle basi degli autospazi", latex: "B = \\bigcup_{i=1}^{r} B_i" },
                { label: "Trucco della dimostrazione", latex: "\\sum_{i=2}^{r} \\sum_{j=1}^{L_i} a_{ji}(\\lambda_i - \\lambda_1)\\vec{v}_{ji} = \\vec{0}" }
            ]
        },
        {
            ...
            subsections: [
                {
                    ...
                    content: `<p>La matrice rappresentativa rispetto alla base canonica $E$ è:</p>
<p>$$A = M_{E,E}(F) = \\begin{pmatrix} 3 & 3 & -1 \\\\ -4 & -2 & 2 \\\\ -2 & 2 & 2 \\end{pmatrix}$$</p>
...
<p>Gli autovalori sono $\\lambda_1 = 0$, $\\lambda_2 = 1$, $\\lambda_3 = 2$. Sono <strong>3 autovalori distinti</strong> in dimensione 3, quindi $F$ è <strong>diagonalizzabile</strong>.</p>`
                },
                {
                    ...
                    content: `<p>Risolviamo $A\\vec{x} = \\vec{0}$. Riduciamo la matrice $A$:</p>
...
<p>$$\\vec{v}_1 = (2, -1, 3) \\qquad V_0 = \\text{Span}(\\vec{v}_1)$$</p>`
                },
                {
                    subtitle: "Autospazio V₁ (λ₂ = 1)",
                    content: `<p>Risolviamo $(A - I)\\vec{x} = \\vec{0}$:</p>
...
<p>$$\\vec{v}_2 = (1, 0, 2) \\qquad V_1 = \\text{Span}(\\vec{v}_2)$$</p>`
                },
                {
                    subtitle: "Autospazio V₂ (λ₃ = 2)",
                    content: `<p>Risolviamo $(A - 2I)\\vec{x} = \\vec{0}$:</p>
...
<p>$$\\vec{v}_3 = (1, 1, 4) \\qquad V_2 = \\text{Span}(\\vec{v}_3)$$</p>`
                },
                {
                    subtitle: "Risultato della diagonalizzazione",
                    content: `<p>La base di autovettori è:</p>
<p>$$B = \\{\\vec{v}_1, \\vec{v}_2, \\vec{v}_3\\} = \\{(2, -1, 3),\\; (1, 0, 2),\\; (1, 1, 4)\\}$$</p>
...
<p>con la relazione $P^{-1}AP = D$.</p>`
            ...
            extra_content: `<p><strong>Verifica rapida.</strong> Si può controllare che $AP = PD$, il che equivale a $P^{-1}AP = D$. Basta verificare che $A\\vec{v}_i = \\lambda_i \\vec{v}_i$ per ciascun autovettore, il che è già garantito dalla costruzione degli autospazi. Ad esempio: $A\\vec{v}_1 = A(2,-1,3) = (0,0,0) = 0 \\cdot \\vec{v}_1$. ✓</p>`,
            formulas: [
...
<p>Le forme bilineari simmetriche generalizzano il concetto di <strong>prodotto scalare</strong>: il prodotto scalare standard su $\\mathbb{R}^n$, definito da $\\langle \\vec{u}, \\vec{v} \\rangle = \\sum_i u_i v_i$, è un caso particolare di forma bilineare simmetrica. In seguito studieremo le <em>forme quadratiche</em>, che si ottengono ponendo i due argomenti uguali: $q(\\vec{v}) = \\phi(\\vec{v}, \\vec{v})$.</p>
...
<p><strong>1. Linearità a sinistra:</strong> Per ogni $\\vec{u}, \\vec{v}, \\vec{w} \\in V$ e $a, b \\in \\mathbb{R}$:</p>
...
<p>Il termine "bilineare" si giustifica perché queste due proprietà insieme implicano anche la <strong>linearità a destra</strong>.</p>`
                },
                {
                    ...
                    content: `<p>Mostriamo che $\\phi$ è anche lineare nel secondo argomento. Per ogni $\\vec{u}, \\vec{v}, \\vec{w} \\in V$ e $a, b \\in \\mathbb{R}$:</p>
<p>$$\\phi(\\vec{u},\\; a\\vec{v} + b\\vec{w}) = \\phi(a\\vec{v} + b\\vec{w},\\; \\vec{u}) \\quad \\text{(simmetria)}$$</p>
<p>$$= a\\,\\phi(\\vec{v}, \\vec{u}) + b\\,\\phi(\\vec{w}, \\vec{u}) \\quad \\text{(linearità a sinistra)}$$</p>
<p>$$= a\\,\\phi(\\vec{u}, \\vec{v}) + b\\,\\phi(\\vec{u}, \\vec{w}) \\quad \\text{(simmetria)}$$</p>
<p>Quindi bastano linearità a sinistra + simmetria per ottenere la bilinearità completa.</p>`
        ...
        },
        {
            ...
            icon: "✏️",
            content: `<p>Consideriamo $\\phi: \\mathbb{R}^2 \\times \\mathbb{R}^2 \\to \\mathbb{R}$ con $\\vec{u} = (x_1, x_2)$ e $\\vec{v} = (y_1, y_2)$:</p>
<p>$$\\phi(\\vec{u}, \\vec{v}) = x_1 y_1 + 3x_1 y_2 + 3x_2 y_1 + 2x_2 y_2$$</p>
<p>Proviamo che è una forma bilineare simmetrica con due metodi.</p>`,
            subsections: [
                {
                    subtitle: "Metodo 1: Verifica diretta",
                    content: `<p><strong>Simmetria:</strong> Scambiando $\\vec{u}$ e $\\vec{v}$:</p>
<p>$$\\phi(\\vec{v}, \\vec{u}) = y_1 x_1 + 3y_1 x_2 + 3y_2 x_1 + 2y_2 x_2 = x_1 y_1 + 3x_2 y_1 + 3x_1 y_2 + 2x_2 y_2 = \\phi(\\vec{u}, \\vec{v}) \\; \\checkmark$$</p>
<p><strong>Linearità a sinistra:</strong> Siano $\\vec{u} = (x_1, x_2)$, $\\vec{v} = (x_1', x_2')$, $\\vec{w} = (y_1, y_2)$ e $a, b \\in \\mathbb{R}$. Allora $a\\vec{u} + b\\vec{v} = (ax_1 + bx_1',\\; ax_2 + bx_2')$ e:</p>
<p>$$\\phi(a\\vec{u} + b\\vec{v},\\; \\vec{w}) = (ax_1 + bx_1')y_1 + 3(ax_1 + bx_1')y_2 + 3(ax_2 + bx_2')y_1 + 2(ax_2 + bx_2')y_2$$</p>
<p>Distribuendo e raggruppando per $a$ e $b$:</p>
<p>$$= a(x_1 y_1 + 3x_1 y_2 + 3x_2 y_1 + 2x_2 y_2) + b(x_1' y_1 + 3x_1' y_2 + 3x_2' y_1 + 2x_2' y_2)$$</p>
<p>$$= a\\,\\phi(\\vec{u}, \\vec{w}) + b\\,\\phi(\\vec{v}, \\vec{w}) \\; \\checkmark$$</p>`
                },
                {
                    subtitle: "Metodo 2: Rappresentazione matriciale",
                    content: `<p>Riscriviamo $\\phi$ in forma matriciale. Raccogliendo:</p>
<p>$$\\phi(\\vec{u}, \\vec{v}) = x_1(y_1 + 3y_2) + x_2(3y_1 + 2y_2) = \\begin{pmatrix} x_1 & x_2 \\end{pmatrix} \\begin{pmatrix} 1 & 3 \\\\ 3 & 2 \\end{pmatrix} \\begin{pmatrix} y_1 \\\\ y_2 \\end{pmatrix}$$</p>
<p>Siano $\\vec{x} = [\\vec{u}]_E$, $\\vec{y} = [\\vec{v}]_E$, $\\vec{z} = [\\vec{w}]_E$ i vettori colonna delle coordinate rispetto alla base canonica $E$, e sia $G = \\begin{pmatrix} 1 & 3 \\\\ 3 & 2 \\end{pmatrix}$. Allora:</p>
<p>$$\\phi(\\vec{u}, \\vec{v}) = \\vec{x}^T G \\vec{y}$$</p>
<p>Ora le proprietà seguono direttamente dall'algebra matriciale:</p>
<p><strong>Linearità a sinistra:</strong></p>
<p>$$\\phi(a\\vec{u} + b\\vec{v},\\; \\vec{w}) = (a\\vec{x} + b\\vec{y})^T G \\vec{z} = (a\\vec{x}^T + b\\vec{y}^T) G \\vec{z} = a(\\vec{x}^T G \\vec{z}) + b(\\vec{y}^T G \\vec{z}) = a\\,\\phi(\\vec{u}, \\vec{w}) + b\\,\\phi(\\vec{v}, \\vec{w})$$</p>
<p><strong>Simmetria:</strong> Il risultato $\\vec{x}^T G \\vec{y}$ è uno scalare (matrice $1 \\times 1$), quindi coincide con la sua trasposta:</p>
<p>$$(\\vec{x}^T G \\vec{y})^T = \\vec{y}^T G^T \\vec{x}$$</p>
<p>Dato che $G$ è simmetrica ($G = G^T$), otteniamo $\\vec{y}^T G \\vec{x} = \\phi(\\vec{v}, \\vec{u})$, quindi $\\phi(\\vec{u}, \\vec{v}) = \\phi(\\vec{v}, \\vec{u})$. ✓</p>`
                },
                {
...
<p><strong>1. Generalità:</strong> Mostra che <em>qualsiasi</em> funzione della forma $\\phi(\\vec{u}, \\vec{v}) = \\vec{x}^T G \\vec{y}$ con $G$ simmetrica è automaticamente una forma bilineare simmetrica. Non serve verificare ogni volta linearità e simmetria "a mano".</p>
...
<p>Il Metodo 1 (verifica diretta) è utile come esercizio didattico per capire cosa significano le proprietà, ma nella pratica il metodo matriciale è più potente e rapido.</p>`
            ...
            formulas: [
                { label: "Forma bilineare dell'esempio", latex: "\\phi(\\vec{u}, \\vec{v}) = \\vec{x}^T G \\vec{y} = \\vec{x}^T \\begin{pmatrix} 1 & 3 \\\\ 3 & 2 \\end{pmatrix} \\vec{y}" }
            ]
        },
        {
            id: "s34-matrice-gram",
            ...
            subsections: [
                {
                    subtitle: "Definizione",
                    content: `<p>Sia $\\phi: V \\times V \\to \\mathbb{R}$ una forma bilineare simmetrica e sia $B = \\{\\vec{e}_1, \\dots, \\vec{e}_n\\}$ una base di $V$. La <strong>matrice di Gram</strong> di $\\phi$ relativa alla base $B$ è la matrice $G \\in \\mathbb{R}^{n \\times n}$ definita da:</p>
<p>$$G_{ij} = \\phi(\\vec{e}_i, \\vec{e}_j), \\qquad i, j = 1, \\dots, n$$</p>
<p>La proprietà fondamentale è che, dati $\\vec{u}, \\vec{v} \\in V$ con coordinate $\\vec{x} = [\\vec{u}]_B$ e $\\vec{y} = [\\vec{v}]_B$ rispetto alla base $B$:</p>
<p>$$\\phi(\\vec{u}, \\vec{v}) = \\vec{x}^T G \\vec{y}$$</p>`
                },
                {
                    subtitle: "Derivazione della formula φ = x^T G y",
                    content: `<p>Questa formula <strong>non è ovvia</strong> e merita una dimostrazione esplicita. Scriviamo i vettori nelle coordinate rispetto a $B$:</p>
<p>$$\\vec{u} = \\sum_{i=1}^{n} x_i \\vec{e}_i, \\qquad \\vec{v} = \\sum_{j=1}^{n} y_j \\vec{e}_j$$</p>
<p>Usando la <strong>bilinearità</strong> (linearità in entrambi gli argomenti):</p>
<p>$$\\phi(\\vec{u}, \\vec{v}) = \\phi\\!\\left(\\sum_{i=1}^{n} x_i \\vec{e}_i,\\; \\sum_{j=1}^{n} y_j \\vec{e}_j\\right) = \\sum_{i=1}^{n} \\sum_{j=1}^{n} x_i \\, y_j \\, \\phi(\\vec{e}_i, \\vec{e}_j)$$</p>
<p>Riconoscendo che $\\phi(\\vec{e}_i, \\vec{e}_j) = G_{ij}$:</p>
<p>$$\\phi(\\vec{u}, \\vec{v}) = \\sum_{i=1}^{n} \\sum_{j=1}^{n} x_i \\, G_{ij} \\, y_j = \\vec{x}^T G \\vec{y}$$</p>
<p>L'ultimo passaggio è proprio la definizione del prodotto matriciale $\\vec{x}^T G \\vec{y}$: la componente $(1,1)$ del risultato è esattamente $\\sum_{i,j} x_i G_{ij} y_j$.</p>`
                },
                {
...
<p>$$G_{ij} = \\phi(\\vec{e}_i, \\vec{e}_j) = \\phi(\\vec{e}_j, \\vec{e}_i) = G_{ji}$$</p>
<p>Questo è un fatto notevole: la matrice di Gram eredita la simmetria dalla forma bilineare.</p>`
  ...
  <p><em>Vettori $\\vec{u}, \\vec{v} \\in V$ → coordinate $\\vec{x} = [\\vec{u}]_B$, $\\vec{y} = [\\vec{v}]_B$ rispetto alla base $B$ → calcolo $\\vec{x}^T G \\vec{y}$ → scalare $\\phi(\\vec{u}, \\vec{v}) \\in \\mathbb{R}$. La matrice $G$ è costruita dalla base $B$: $G_{ij} = \\phi(\\vec{e}_i, \\vec{e}_j)$.</em></p>
  <p style="margin-top: 8px; font-size: 0.75rem; color: var(--accent);">[ immagine da inserire ]</p>
</div>`,
            formulas: [
                { label: "Entrate della matrice di Gram", latex: "G_{ij} = \\phi(\\vec{e}_i, \\vec{e}_j)" },
                { label: "Formula fondamentale", latex: "\\phi(\\vec{u}, \\vec{v}) = \\vec{x}^T G \\vec{y}" },
...
<p><strong>1. Cambio di base:</strong> Se si cambia base da $B$ a $B'$ con matrice di passaggio $M$, la matrice di Gram si trasforma come $G' = M^T G M$ (formula di congruenza, diversa dalla similitudine $P^{-1}AP$ degli endomorfismi!).</p>
<p><strong>2. Forme quadratiche:</strong> Ponendo $\\vec{v} = \\vec{u}$, si ottiene $q(\\vec{u}) = \\phi(\\vec{u}, \\vec{u}) = \\vec{x}^T G \\vec{x}$, una funzione da $V$ a $\\mathbb{R}$.</p>
    ...
    oral_cards: [
        {
            ...
            back: "Se $\\lambda_1, \\dots, \\lambda_r$ sono autovalori distinti di $F$ e per ogni $i$ si sceglie un insieme $B_i$ di autovettori l.i. relativi a $\\lambda_i$, allora $B = \\bigcup B_i$ è l.i. La dimostrazione è per induzione su $r$: si parte da una combinazione lineare nulla $\\sum a_{ji}\\vec{v}_{ji} = \\vec{0}$, si applica $F$ per ottenere $\\sum a_{ji}\\lambda_i\\vec{v}_{ji} = \\vec{0}$, si sottrae la prima moltiplicata per $\\lambda_1$ per eliminare i termini con $i=1$, e per ipotesi induttiva i coefficienti per $i \\geq 2$ sono nulli (perché $\\lambda_i - \\lambda_1 \\neq 0$). Sostituendo, anche i coefficienti per $i=1$ risultano nulli."
        },
        {
            type: "tranello",
        ...
        },
        {
            ...
            back: "È una funzione $\\phi: V \\times V \\to \\mathbb{R}$ che soddisfa: (1) linearità a sinistra: $\\phi(a\\vec{u} + b\\vec{v}, \\vec{w}) = a\\phi(\\vec{u},\\vec{w}) + b\\phi(\\vec{v},\\vec{w})$; (2) simmetria: $\\phi(\\vec{u},\\vec{v}) = \\phi(\\vec{v},\\vec{u})$. Da queste due segue anche la linearità a destra, quindi $\\phi$ è lineare in entrambi gli argomenti. Il prodotto scalare standard è un caso particolare."
        },
        {
            type: "definizione",
            front: "Cos'è la matrice di Gram di una forma bilineare simmetrica φ rispetto a una base B?",
            back: "Data una base $B = \\{\\vec{e}_1, \\dots, \\vec{e}_n\\}$, la matrice di Gram è $G \\in \\mathbb{R}^{n \\times n}$ con $G_{ij} = \\phi(\\vec{e}_i, \\vec{e}_j)$. È sempre simmetrica ($G = G^T$). La proprietà fondamentale è che $\\phi(\\vec{u}, \\vec{v}) = \\vec{x}^T G \\vec{y}$, dove $\\vec{x}$ e $\\vec{y}$ sono le coordinate di $\\vec{u}$ e $\\vec{v}$ nella base $B$."
        },
        {
            type: "dimostrazione",
            front: "Dimostra che φ(u,v) = x^T G y, dove G è la matrice di Gram.",
            back: "Si scrive $\\vec{u} = \\sum_i x_i \\vec{e}_i$ e $\\vec{v} = \\sum_j y_j \\vec{e}_j$. Per bilinearità: $\\phi(\\vec{u},\\vec{v}) = \\sum_{i,j} x_i y_j \\phi(\\vec{e}_i, \\vec{e}_j) = \\sum_{i,j} x_i G_{ij} y_j$. Questa doppia somma è esattamente il prodotto matriciale $\\vec{x}^T G \\vec{y}$."
        },
        {
            type: "domanda",
            front: "Perché dalla linearità a sinistra e dalla simmetria segue la linearità a destra?",
            back: "Si usa la simmetria per 'spostare' l'argomento che vogliamo trattare: $\\phi(\\vec{u}, a\\vec{v}+b\\vec{w}) = \\phi(a\\vec{v}+b\\vec{w}, \\vec{u})$ (simmetria) $= a\\phi(\\vec{v},\\vec{u}) + b\\phi(\\vec{w},\\vec{u})$ (linearità a sinistra) $= a\\phi(\\vec{u},\\vec{v}) + b\\phi(\\vec{u},\\vec{w})$ (simmetria). Tre passaggi, tutti giustificati."
        },
        {
            type: "formula",
        ...
        },
        {
            ...
            back: "Perché una volta scritto $\\phi(\\vec{u},\\vec{v}) = \\vec{x}^T G \\vec{y}$, basta controllare che $G = G^T$ per avere la simmetria, e la linearità è automatica (proprietà dell'algebra matriciale). Il risultato è generale: qualsiasi funzione della forma $\\vec{x}^T G \\vec{y}$ con $G$ simmetrica è una forma bilineare simmetrica."
        },
        {
            type: "tranello",
