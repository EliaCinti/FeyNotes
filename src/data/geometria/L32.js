...
<p><strong>Esempio fondamentale:</strong> Data una matrice $A \\in \\mathcal{M}(n, n)$, l'applicazione $L_A: \\mathbb{R}^n \\to \\mathbb{R}^n$ definita da $L_A(\\mathbf{x}) = A\\mathbf{x}$ è un endomorfismo. La sua matrice rappresentativa rispetto alla base canonica $\\mathcal{E}$ è proprio $A$: $M_{\\mathcal{E}}^{\\mathcal{E}}(L_A) = A$.</p>`,
            subsections: [
                {
                    subtitle: "Autovalori e autovettori",
                    content: `<p>Sia $f: V \\to V$ un endomorfismo e $\\lambda \\in \\mathbb{R}$. Diciamo che $\\lambda$ è un <strong>autovalore</strong> per $f$ se esiste $\\mathbf{v} \\in V$, con $\\mathbf{v} \\neq \\mathbf{0}$, tale che:</p>
<p>$$f(\\mathbf{v}) = \\lambda \\mathbf{v}$$</p>
<p>Il vettore $\\mathbf{v}$ si chiama <strong>autovettore</strong> di $f$ associato all'autovalore $\\lambda$.</p>`
                },
                {
...
<p>$$V_\\lambda = \\{\\mathbf{v} \\in V \\mid f(\\mathbf{v}) = \\lambda \\mathbf{v}\\}$$</p>
...
<p>$$V_\\lambda = \\ker(f - \\lambda \\text{id}_V)$$</p>
<p><strong>Dimostrazione:</strong> $\\mathbf{v} \\in V_\\lambda$ se e solo se $f(\\mathbf{v}) = \\lambda \\mathbf{v}$, cioè $f(\\mathbf{v}) - \\lambda \\mathbf{v} = \\mathbf{0}$, cioè $(f - \\lambda \\text{id}_V)(\\mathbf{v}) = \\mathbf{0}$, cioè $\\mathbf{v} \\in \\ker(f - \\lambda \\text{id}_V)$. Poiché il nucleo di un operatore lineare è sempre un sottospazio, anche $V_\\lambda$ lo è. $\\square$</p>`
                },
                {
...
<p>$$m_g(\\lambda) = \\dim(\\ker(f - \\lambda \\text{id}_V)) = n - \\operatorname{rank}(A - \\lambda I)$$</p>
...
<p>Prendiamo una base $\\{\\mathbf{v}_1, \\dots, \\mathbf{v}_h\\}$ per $V_\\lambda$ e completiamola a una base di tutto $V$:</p>
<p>$$\\mathcal{B} = \\{\\mathbf{v}_1, \\dots, \\mathbf{v}_h, \\mathbf{v}_{h+1}, \\dots, \\mathbf{v}_n\\}$$</p>
<p>Per i primi $h$ vettori si ha $f(\\mathbf{v}_i) = \\lambda \\mathbf{v}_i$, quindi la matrice $M_{\\mathcal{B}}^{\\mathcal{B}}(f)$ ha la struttura a blocchi:</p>
...
<p><strong>Lemma:</strong> Siano $\\lambda_1, \\dots, \\lambda_k$ autovalori <strong>distinti</strong> di un endomorfismo $f$, e per ogni $i$ sia $\\{\\mathbf{v}_1^{(i)}, \\dots, \\mathbf{v}_{r_i}^{(i)}\\}$ una base dell'autospazio $V_{\\lambda_i}$. Allora l'unione di tutte queste basi è un insieme di vettori <strong>linearmente indipendenti</strong>.</p>
<p><strong>Dimostrazione (caso semplificato con un autovettore per autovalore):</strong> Mostriamo il caso in cui prendiamo un singolo autovettore $\\mathbf{v}_i$ per ciascun autovalore $\\lambda_i$. Procedendo per induzione su $k$:</p>
<p><strong>Base ($k=1$):</strong> Un singolo autovettore $\\mathbf{v}_1 \\neq \\mathbf{0}$ è banalmente linearmente indipendente.</p>
<p><strong>Passo induttivo:</strong> Supponiamo che $\\mathbf{v}_1, \\dots, \\mathbf{v}_{k-1}$ (relativi a $\\lambda_1, \\dots, \\lambda_{k-1}$ distinti) siano linearmente indipendenti. Consideriamo una combinazione:</p>
<p>$$c_1 \\mathbf{v}_1 + c_2 \\mathbf{v}_2 + \\dots + c_k \\mathbf{v}_k = \\mathbf{0} \\qquad (*)$$</p>
<p>Applichiamo $f$ a entrambi i membri:</p>
<p>$$c_1 \\lambda_1 \\mathbf{v}_1 + c_2 \\lambda_2 \\mathbf{v}_2 + \\dots + c_k \\lambda_k \\mathbf{v}_k = \\mathbf{0} \\qquad (**)$$</p>
<p>Moltiplichiamo $(*)$ per $\\lambda_k$:</p>
<p>$$c_1 \\lambda_k \\mathbf{v}_1 + c_2 \\lambda_k \\mathbf{v}_2 + \\dots + c_k \\lambda_k \\mathbf{v}_k = \\mathbf{0} \\qquad (***)$$</p>
<p>Sottraendo $(***)$ da $(**)$:</p>
<p>$$c_1(\\lambda_1 - \\lambda_k)\\mathbf{v}_1 + c_2(\\lambda_2 - \\lambda_k)\\mathbf{v}_2 + \\dots + c_{k-1}(\\lambda_{k-1} - \\lambda_k)\\mathbf{v}_{k-1} = \\mathbf{0}$$</p>
<p>Per ipotesi induttiva, $\\mathbf{v}_1, \\dots, \\mathbf{v}_{k-1}$ sono linearmente indipendenti, quindi $c_i(\\lambda_i - \\lambda_k) = 0$ per ogni $i = 1, \\dots, k-1$. Poiché gli autovalori sono <strong>distinti</strong>, $\\lambda_i - \\lambda_k \\neq 0$, dunque $c_i = 0$ per $i = 1, \\dots, k-1$. Tornando a $(*)$: $c_k \\mathbf{v}_k = \\mathbf{0}$, e poiché $\\mathbf{v}_k \\neq \\mathbf{0}$, si ha $c_k = 0$. $\\square$</p>
...
<p>$$V = V_{\\lambda_1} \\oplus V_{\\lambda_2} \\oplus \\dots \\oplus V_{\\lambda_k}$$</p>`
                },
                {
                    ...
                    content: `<p>$0$ è un autovalore di $f$ se e solo se $\\ker(f) \\neq \\{\\mathbf{0}\\}$. In tal caso, l'autospazio $V_0$ coincide con il nucleo: $V_0 = \\ker(f)$.</p>`
                }
            ]
        },
        {
...
<li><strong>Costruire la base diagonalizzante:</strong> per ogni $\\lambda_i$, trovare una base $\\mathcal{A}_i$ di $V_{\\lambda_i}$ risolvendo il sistema omogeneo $(A - \\lambda_i I)\\mathbf{x} = \\mathbf{0}$. L'unione $\\mathcal{A} = \\bigcup \\mathcal{A}_i$ è la base di autovettori.</li>
<li><strong>Costruire $P$ e $D$:</strong> la matrice $P$ ha come colonne gli autovettori (ordinati coerentemente), la matrice $D$ ha gli autovalori sulla diagonale nello stesso ordine. Si ha $D = P^{-1}AP$.</li>
                ...
                },
                {
                    ...
                    content: `<p>Se gli autovalori distinti sono $\\lambda_1, \\dots, \\lambda_k$ con basi $\\{\\mathbf{v}_1^{(i)}, \\dots, \\mathbf{v}_{m_i}^{(i)}\\}$ per $V_{\\lambda_i}$, le matrici hanno la forma:</p>
<p>$$P = \\begin{pmatrix} | & | & & | & | & & | \\\\ \\mathbf{v}_1^{(1)} & \\cdots & \\cdots & \\mathbf{v}_{m_1}^{(1)} & \\mathbf{v}_1^{(2)} & \\cdots & \\mathbf{v}_{m_k}^{(k)} \\\\ | & | & & | & | & & | \\end{pmatrix}$$</p>
...
<p><strong>La matrice è diagonalizzabile!</strong></p>`
                },
                {
                    ...
                    content: `<p><strong>Autospazio $V_2$:</strong> Risolviamo $(A - 2I)\\mathbf{x} = \\mathbf{0}$:</p>
...
<p><strong>Autospazio $V_3$:</strong> Risolviamo $(A - 3I)\\mathbf{x} = \\mathbf{0}$:</p>
...
<p>Il professore si aspetta che sappiate citare il lemma sugli autovettori di autovalori distinti e che la dimostrazione della proposizione sulle molteplicità sia presentata correttamente.</p>`
    ...
    oral_cards: [
        ...
        },
        {
            ...
            back: "La molteplicità geometrica $m_g(\\lambda)$ è la dimensione dell'autospazio $V_\\lambda = \\ker(f - \\lambda \\text{id}_V)$. Si calcola come $m_g(\\lambda) = n - \\operatorname{rank}(A - \\lambda I)$, dove $A$ è una matrice rappresentativa e $n = \\dim(V)$."
        },
        {
            type: "formula",
        ...
        },
        {
            ...
            back: "Si prende una base $\\{\\mathbf{v}_1, \\dots, \\mathbf{v}_h\\}$ di $V_\\lambda$ ($h = m_g(\\lambda)$) e la si completa a una base $\\mathcal{B}$ di $V$. La matrice $M_{\\mathcal{B}}^{\\mathcal{B}}(f)$ ha la forma a blocchi triangolare superiore $\\begin{pmatrix} \\lambda I_h & C \\\\ 0 & E \\end{pmatrix}$. Il polinomio caratteristico diventa $p_f(t) = (\\lambda - t)^h \\cdot \\det(E - tI_{n-h})$, quindi $(t-\\lambda)^h$ divide $p_f(t)$ e $h \\le m_a(\\lambda)$."
        },
        {
            type: "domanda",
        ...
        },
        {
            ...
            back: "Per induzione. Se $c_1\\mathbf{v}_1 + \\dots + c_k\\mathbf{v}_k = \\mathbf{0}$ con $f(\\mathbf{v}_i) = \\lambda_i\\mathbf{v}_i$, si applica $f$ e si sottrae $\\lambda_k$ volte l'equazione originale: $\\sum_{i=1}^{k-1} c_i(\\lambda_i - \\lambda_k)\\mathbf{v}_i = \\mathbf{0}$. Per ipotesi induttiva e $\\lambda_i \\neq \\lambda_k$, si ha $c_i = 0$ per ogni $i$, e quindi anche $c_k = 0$."
        },
        {
            type: "tranello",
        ...
        },
        {
            ...
            back: "$0$ è autovalore di $f$ se e solo se $\\ker(f) \\neq \\{\\mathbf{0}\\}$, cioè se e solo se $f$ non è iniettiva. In tal caso, l'autospazio $V_0$ coincide esattamente con $\\ker(f)$."
        },
        {
            type: "formula",
